...
const Features = () => {
  return (
    <section id="features" className="py-20 relative overflow-hidden">
      ...
      {/* Decorative circles */}
      <div className="absolute left-0 top-0 w-64 h-64 bg-purple-100 rounded-full mix-blend-multiply filter blur-3xl opacity-70"></div>
      <div className="absolute right-0 bottom-0 w-64 h-64 bg-indigo-100 rounded-full mix-blend-multiply filter blur-3xl opacity-70"></div>
      ...
      <div className="container px-4 mx-auto relative">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {features.map((feature, index) => (
            <div 
              key={index} 
              className="relative card-glass rounded-xl p-8 transition-all duration-300 hover:shadow-xl group"
              style={{animationDelay: `${index * 0.2}s`}}
            >
              <div className="absolute bottom-0 left-0 right-0 h-1 bg-gradient-to-r from-primary/50 to-primary rounded-b-xl transform scale-x-0 group-hover:scale-x-100 transition-transform duration-300 origin-left"></div>
